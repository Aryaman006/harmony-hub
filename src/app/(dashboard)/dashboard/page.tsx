...
// Mock chart data for when API data isn't available
const mockAreaData = [
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/integration/dashboard/stats`, {
        headers: {
          'x-api-key': 'TRAVELHEALTH_SECURE_KEY',
        },
      });
      ...
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
        {stats.map((stat: any, i: number) => {
          ...
          return (
            <div
              key={stat.name}
              className="group relative overflow-hidden rounded-xl bg-card border border-border p-5 transition-all duration-300 hover:shadow-lg hover:shadow-primary/5 hover:-translate-y-0.5"
            >
        ...
        <div className="space-y-3">
          {latestActivities.length > 0 ? (
            latestActivities.map((activity: any) => (
              <div
                key={activity.id}
                className="flex items-center justify-between rounded-lg border border-border/50 bg-secondary/30 p-3.5 transition-colors hover:bg-secondary/60"
              >
                ...
                <span className="text-[11px] font-semibold text-accent-foreground bg-accent rounded-full px-2.5 py-1">
                  {activity.event}
                </span>
              </div>
            ))
